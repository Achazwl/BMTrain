...
class DistributedLinear(bmt.DistributedModule):
    def __init__(self, dim_in : int, dim_out : int,):
        super().__init__()
        self.dim_in = dim_in
        self.weight = bmt.DistributedParameter( torch.empty((dim_out, dim_in), dtype=torch.float16) )
        self.bias = bmt.DistributedParameter( torch.empty((dim_out,), dtype=torch.float16) )

    def forward(self, x : torch.Tensor):
        x = torch.nn.funtional.linear(x, self.weight, self.bias)
        return x

class DistModule(bmt.DistributedModule):
    def __init__(self):
        super().__init__()
        self.fc1 = DistributedLinear(128, 128)
        self.fc2 = DistributedLinear(128, 128)
        self.fc3 = DistributedLinear(128, 128)
        self.fc4 = DistributedLinear(128, 128)
        self.fc5 = DistributedLinear(128, 128)
...
def main():
    ...
    opt1 = bmt.optim.LambOffloadOptimizer(model1.parameters(), weight_decay=1e-3, scale=1)
    opt2 = bmt.optim.LambOptimizer(model2.parameters(), weight_decay=1e-3, scale=1)
    ...
    for _ in range(100):
        ...
        for p1, p2, p3 in zip(model1.parameters(), model2.parameters(), model3.parameters()):
            grad = torch.randn_like(p3, dtype=torch.float16)
            numel = grad.numel() // bmt.world_size()
            p1.grad = grad.view(-1)[bmt.rank()*numel:(bmt.rank()+1)*numel].view_as(p1)
            p2.grad = grad.view(-1)[bmt.rank()*numel:(bmt.rank()+1)*numel].view_as(p2)
            p3.grad = grad.float()
        ...
        for _p1, _p2, p3 in zip(model1.parameters(), model2.parameters(), model3.parameters()):
            p1 = torch.zeros_like(p3, dtype=torch.float16)
            p2 = torch.zeros_like(p3, dtype=torch.float16)
            bmt.nccl.allGather(_p1.storage(), p1.storage(), config["comm"])
            bmt.nccl.allGather(_p2.storage(), p2.storage(), config["comm"])
            p1.view_as(p3)
            p2.view_as(p3)
